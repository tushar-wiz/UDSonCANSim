...
computer_image = tk.CTkImage(Image.open(
    "./media/computer.png"), size=(190, 190))
...
frame_progress = tk.CTkProgressBar(
    top_frame, variable=frame_progress_var, orientation='horizontal', mode='determinate')
...
SID_dropdown = tk.CTkOptionMenu(
    SID_frame, variable=SID_key, values=SID_options, width=95)
...
DID_SUB_dropdown = tk.CTkOptionMenu(
    DID_SUB_frame, variable=DID_SUB_key, values=DID_SUB_options, width=65, fg_color="#1D1E1E")
...
EXAMPLE_options = ["Session Control", "Single Frame RDBI"]
EXAMPLE_dropdown = tk.CTkOptionMenu(
    example_frame, variable=EXAMPLE_key, values=EXAMPLE_options, width=250)
...
MANUAL_EXAMPLE_button = tk.CTkSegmentedButton(
    right_frame, values=MANUAL_EXAMPLE_options, variable=MANUAL_EXAMPLE_key_ext, command=MANUAL_EXAMPLE_func)
    ...
    if(flagS):
        if sv.fr[1] == 0x10:
            if sv.fr[2] == 0x02 and sv.currentSession == 0x01:
                # defaultSession to programmingSession is unallowed
